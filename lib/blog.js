...
adminConfigSuccess = function(template)
{
    return function(config){
        locals = {"adminSubtitle":__("Blog configuration"),"config":config};
        return viewEngine.respond('admin_config_'+template+'.html', {locals:locals,layout:"admin_layout.html",partials:partials.list});
    };     
};

adminPostSuccess = function(template)
{
...
router.get('/admin/config/settings', function(req) {
    return helper.locales().then (function (locales) {
        return helper.config().then (function (config) {
            var localeDicts = [];
            for (k in locales) {
                locale=locales[k];
                localeDicts.push({'locale':locale,'selected':locale==config.locale,'name':__('locale_'+locale)});
            }
            config['locales']=localeDicts;
            return adminConfigSuccess("settings")(config);
        },
        adminError());
    },
    adminError());
...
router.get('/admin/config/css', function(req) {
    return helper.config().then (
        adminConfigSuccess("css"),
        adminError());
});

router.get('/admin/config/disclaimer', function(req) {
    return helper.config().then (
        adminConfigSuccess("disclaimer"),
        adminError());
});

router.get('/admin/config/images', function(req) {
    return helper.config().then (
        adminConfigSuccess("images"),
        adminError());
});
...
router.get('/admin/segmentSkel/:type', function(req) {
    ...
    return viewEngine.respond('admin_post_segments.html', {locals:locals,layout:'partials/admin_segment.html'});
});

router.get('/admin/post/:id/attachments', function(req) {
  return couchdb.get(req.params.id).then(
        adminPostSuccess("attachments"), 
        adminError()
  );
});
...
router.post('/admin/config/settings', function(req) {
    ...
    mergeDict.locale = req.params.locale;
