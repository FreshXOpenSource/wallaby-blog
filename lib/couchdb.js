...
function get (docID) {
    ...
    try {
        database.get(docID, function (err, doc) {
            if (err) {
                sys.puts("get("+docID+"): "+JSON.stringify(err));
                defer.reject(err);
            } else
                defer.resolve(doc);
        });
    } catch (e) {
        sys.puts("Exception in couchdb.get: "+e);
        defer.reject(e);
    }
...
exports.save = function (doc) {
    ...
    try {
        database.save(doc, function (err, res) {
            if (err) {
                sys.puts("save("+doc+"): "+JSON.stringify(err));
                defer.resolve();
            } else
                defer.resolve(res);
        });
    } catch (e) {
        sys.puts("Exception in couchdb.save: "+e);
        defer.reject(e);
    }
...
exports.view = function (viewID,options) {
    ...
    try {
        database.view(viewID,options, function (err, res) {
            if (err) {
                sys.puts("view("+viewID+"): "+JSON.stringify(err));
                defer.reject(err);
            } else
                defer.resolve(res);
        });
    } catch (e) {
        sys.puts("Exception in couchdb.view: "+e);
        defer.reject(e);
    }
...
exports.merge = function (docID, mergeDict) {
    ...
    try {
        database.merge(docID, mergeDict, function (err, res) {
            if (err) {
                if (err.error == "not_found") {
                    ...
                    sys.puts("merge("+docID+"): "+JSON.stringify(err));
                    defer.resolve();
                }
            } else
                defer.resolve(res);
        ...
        sys.puts("Exception in couchdb.merge: "+e);
        defer.reject(e);
    }
    ...
    try {
        get(docID).then(function (doc) {
            database.removeAttachment(doc, attachment, function (err, res) {
                if (err) {
                    sys.puts("deleteFile("+docID+","+attachment+"): "+JSON.stringify(err));
                    defer.reject(err);
                } else
                    defer.resolve();
            });
        ...
        sys.puts("Exception in couchdb.deleteFile: "+e);
        defer.reject(e);
    }
...
function uploadMultipleFiles (docID, attachments) {
    ...
    try {
            ...
            async.forEachSeries(attachments, function(queuedAttachment, callback) {
                if (queuedAttachment['file'] == undefined || queuedAttachment['file'].length == 0) {
                    ...
                    return fs.readFile(queuedAttachment['file'], function (err,data) {
                        if (err) {
                            sys.puts("could not read file: "+queuedAttachment['file']);
                            callback();
                        } else {
                            ...
                            database.saveAttachment(doc, attachment, function (err, res) {
                                if (err) {
                                    sys.puts("uploadMultipleFiles("+docID+","+attachment+"): "+JSON.stringify(err));
                                    defer.reject(err);
                                } else {
                                    doc['rev']=res['rev'];
                                }
        ...
        sys.puts("Exception in couchdb.uploadMultipleFiles: "+e);
        defer.reject(e);
    }
...
exports.createDatabase = function () {
    ...
    try {
        database.create(function (err,res) {
            if (err) {
                sys.puts("createDatabase(): "+JSON.stringify(err));
                defer.reject(err);
            } else {
                defer.resolve();
            }
        ...
        sys.puts("Exception in couchdb.createDatabase: "+e);
        defer.reject(e);
    }
