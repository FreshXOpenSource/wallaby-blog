...
function get (docID) {
    ...
    try {
        database.get(docID, function (err, doc) {
            if (err) {
                util.log("get("+docID+"): "+JSON.stringify(err));
                defer.reject(err);
            } else
                defer.resolve(doc);
        });
    } catch (e) {
        util.log("Exception in couchdb.get: "+e);
        defer.reject(e);
    }
...
exports.save = function (doc) {
    ...
    try {
        database.save(doc, function (err, res) {
            if (err) {
                util.log("save("+doc+"): "+JSON.stringify(err));
                defer.resolve();
            } else
                defer.resolve(res);
        });
    } catch (e) {
        util.log("Exception in couchdb.save: "+e);
        defer.reject(e);
    }
...
exports.view = function (viewID,options) {
    ...
    try {
        database.view(viewID,options, function (err, res) {
            if (err) {
                util.log("view("+viewID+"): "+JSON.stringify(err));
                defer.reject(err);
            } else
                defer.resolve(res);
        });
    } catch (e) {
        util.log("Exception in couchdb.view: "+e);
        defer.reject(e);
    }
...
exports.merge = function (docID, mergeDict) {
    ...
    try {
        database.merge(docID, mergeDict, function (err, res) {
            if (err) {
                if (err.error == "not_found") {
                    ...
                    util.log("merge("+docID+"): "+JSON.stringify(err));
                    defer.resolve();
                }
            } else
                defer.resolve(res);
        ...
        util.log("Exception in couchdb.merge: "+e);
        defer.reject(e);
    }
    ...
    try {
        get(docID).then(function (doc) {
            database.removeAttachment(doc, attachment, function (err, res) {
                if (err) {
                    util.log("deleteFile("+docID+","+attachment+"): "+JSON.stringify(err));
                    defer.reject(err);
                } else
                    defer.resolve();
            });
        ...
        util.log("Exception in couchdb.deleteFile: "+e);
        defer.reject(e);
    }
...
function uploadMultipleFiles (docID, attachments) {
    ...
    try {
            ...
            async.forEachSeries(attachments, function(queuedAttachment, callback) {
                if (queuedAttachment['file'] == undefined || queuedAttachment['file'].length == 0) {
                    ...
                    return fs.readFile(queuedAttachment['file'], function (err,data) {
                        if (err) {
                            util.log("could not read file: "+queuedAttachment['file']);
                            callback();
                        } else {
                            ...
                            database.saveAttachment(doc, attachment, function (err, res) {
                                if (err) {
                                    util.log("uploadMultipleFiles("+docID+","+attachment+"): "+JSON.stringify(err));
                                    defer.reject(err);
                                } else {
                                    doc['rev']=res['rev'];
                                }
        ...
        util.log("Exception in couchdb.uploadMultipleFiles: "+e);
        defer.reject(e);
    }
...
exports.createDatabase = function () {
    ...
    try {
        database.create(function (err,res) {
            if (err) {
                util.log("createDatabase(): "+JSON.stringify(err));
                defer.reject(err);
            } else {
                defer.resolve();
            }
        ...
        util.log("Exception in couchdb.createDatabase: "+e);
        defer.reject(e);
    }
