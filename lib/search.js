...
function search (keyword) {
    ...
    try {
        ...
        util.log("Exception in search.search: "+JSON.stringify(e));
        defer.reject(e);
    }
            ...
            deleteRiver().then(function (res) {
                deleteIndex().then(function (res) {
                    putRiver().then(function (res) {
                        util.log("[ElasticSearch] River update success: "+JSON.stringify(res));
                    }, function (err) {
                        util.log("[ElasticSearch] Error updating river: "+JSON.stringify(err));
                    });
                }, function (err) {
                    util.log("[ElasticSearch] Error deleting index: "+JSON.stringify(err));
                });
            }, function (err) {
                util.log("[ElasticSearch] Error deleting river: "+JSON.stringify(err));
            });
        } else {
            util.log("[ElasticSearch] River up to date!");
        }
    }, function (err) {
        putRiver().then(function (res) {
            util.log("[ElasticSearch] River install success: "+JSON.stringify(res));
        }, function (err) {
            util.log("[ElasticSearch] Error installing river: "+JSON.stringify(err));
        });
    });
}
