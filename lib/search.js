...
function search (keyword) {
    ...
    try {
        ...
        sys.puts("Exception in search.search: "+JSON.stringify(e));
        defer.reject(e);
    }
            ...
            deleteRiver().then(function (res) {
                deleteIndex().then(function (res) {
                    putRiver().then(function (res) {
                        sys.puts("[ElasticSearch] River update success: "+JSON.stringify(res));
                    }, function (err) {
                        sys.puts("[ElasticSearch] Error updating river: "+JSON.stringify(err));
                    });
                }, function (err) {
                    sys.puts("[ElasticSearch] Error deleting index: "+JSON.stringify(err));
                });
            }, function (err) {
                sys.puts("[ElasticSearch] Error deleting river: "+JSON.stringify(err));
            });
        } else {
            sys.puts("[ElasticSearch] River up to date!");
        }
    }, function (err) {
        putRiver().then(function (res) {
            sys.puts("[ElasticSearch] River install success: "+JSON.stringify(res));
        }, function (err) {
            sys.puts("[ElasticSearch] Error installing river: "+JSON.stringify(err));
        });
    });
}
