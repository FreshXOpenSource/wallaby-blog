...
exports.readFile = function (file) {
    ...
    try {
        fs.readFile(file, function (err,data) {
            if (err) {
                sys.puts("could not read file("+file+"): "+err);
        ...
        sys.puts('Could not read file: '+e);
        defer.resolve(undefined);
    }
...
exports.resizeImage = function (file) {
    ...
    try {
        ...
        sys.puts('Could not resize image: '+e);
        defer.resolve(file);
    }
...
function preparePost (post) {
    try {
        ...
        sys.puts('Could not prepare post: '+e+" ("+JSON.stringify(post)+")");
    }
            ...
            return couchdb.view(path,countOptions).then(function(countRes) {
                return couchdb.view(path,options).then(function(res) {
                    ...
                    return counts().then(function(counts) {
                        ...
                        return viewEngine.respond('posts.html', {locals:locals,partials:{'post':'post.partial.html'}});
                    });
                });
            });
        } catch (e) {
            sys.puts('Could not load posts: '+e);
...
function counts () {
    return couchdb.view('blog/counts',{"descending":true}).then(function(res) {
        try {
            ...
            sys.puts('Could not load counts: '+e);
            return {};
        }
    });
}
